function [thetabold ActSet ExpertP TestEst TestEst2]=MGPC_Sigmoid_ActSet(TrainX,TrainY,TestX,TestY, T, MaxOuterIteration, MaxEMite, MaxIteration_VI, MaxIteration_CG)
% ????????????D*N
rand('state',1); %??????????????
%-----------------------------------------------preprocessing
...
M_Cand=zeros(T,1); % T: maximum # of GPs; Mupper: maximum # of active set points for one GP
ActCandSize=N;         %randomly pick 100 examples as the actset candidate
ActCand=ones(T,ActCandSize);
for ii=1:T
    temp=size(ActSet{ii},1);
    M_Cand(ii) = temp;
